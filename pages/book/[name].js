...
export default function Book() {
  ...
  if (!router.isReady) return <></>;
	let chapters = [];
  let chapterName = name.toLowerCase();
  if (chapterName === "virtue") {
    chapters = virtueChapters;
  } else if (chapterName === "wealth") {
    chapters = wealthChapters;
  } else if (chapterName === "love") {
  ...
  return (
    <>
      <Header title={`Book of ${titleCase(chapterName)}`} />
      <ul className={styles.chapter_name_list}>
        {chapters.map((chapter, index) => {
          return (
            <li key={index} className={styles.chapter_name_list_item}>
              <Link href={`/kural/${(index) * 10 + 1}`} className={styles.chapter_link}>
